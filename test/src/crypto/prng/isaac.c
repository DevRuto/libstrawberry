...
sb_bool_t test_isaac() {
	...
	for (i = 0, k = 0; i < 2; ++i) {
		...
		for (j = 0; j < SB_CRYPTO_PRNG_ISAAC_SIZE; ++j, ++k) {
			if (ctx.rsl[j] != vectors[k]) {
				valid = sb_false;
				status("isaac", "http://burtleburtle.net/bob/rand/randvect.txt", status_failed);
			} else {
				status("isaac", "http://burtleburtle.net/bob/rand/randvect.txt", status_passed);
			}
		}
	}
